...
class TestEditableTable:
    ...
    def test_create_button_in_html(self):
        """Verify Create New Table button is in HTML panel."""
        from figrecipe._editor._templates._html_datatable import HTML_DATATABLE_PANEL

        assert "btn-create-csv" in HTML_DATATABLE_PANEL
        assert "Create New Table" in HTML_DATATABLE_PANEL
        assert "createNewCSV()" in HTML_DATATABLE_PANEL

...
def _format_annotate(id, tracked_dict, kwargs):
    ...
    if len(args) >= 2:
        ...
        data = {
            ...
            get_csv_column_name("content", ax_row, ax_col, trace_id=trace_id): [
                text_content
            ],
        }
        ...
        if xytext is not None and hasattr(xytext, "__len__") and len(xytext) >= 2:
            data[get_csv_column_name("text_x", ax_row, ax_col, trace_id=trace_id)] = [
                xytext[0]
            ]
            data[get_csv_column_name("text_y", ax_row, ax_col, trace_id=trace_id)] = [
                xytext[1]
            ]
